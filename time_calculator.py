def add_time(start, duration, day_of_week=False):
  ...
  no_of_am_pm_flips=int((start_hours+duration_hours)/12)
  ...
  am_or_pm = am_and_pm_flip[am_or_pm] if no_of_am_pm_flips % 2 == 1 else am_or_pm

  new_time= str(end_hours)+ ":" + str(end_minutes) + " " + am_or_pm
  if(day_of_week):
    day_of_week=day_of_week.lower()
    index=int((week_days_index[day_of_week])+ no_of_days) %7
  ...
  if(no_of_days==1):
    ...
    return new_time + " (" +str(no_of_days) + " days later)"
